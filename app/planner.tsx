...
function MissionEditor({ record, save }: {
    ...
    setBusy(false);
} }}><label className="field">Moje poslanie<textarea className="mission-input" value={r.mission || ''} onChange={e => setR({ ...r, mission: e.target.value })} placeholder="Chcem byť človekom, ktorý…"/></label><label className="field">Moja vízia do budúcnosti<textarea value={r.vision || ''} onChange={e => setR({ ...r, vision: e.target.value })} placeholder="Keď sa raz obzriem späť, chcem vidieť…"/></label>{error && <p className="error">{error}</p>}<button className="btn primary" disabled={busy}><Save size={16}/>{busy ? 'Ukladám…' : 'Uložiť môj kompas'}</button></form>; }
